...
- Canonical intents are the durable input. Reconciliation derives each subject's aggregate temporal state and applies the selected Entra group assertions.
- PostgreSQL owns intent, schedule, and reconciliation state. Exercise transaction, locking, migration, and query semantics against PostgreSQL with `mise run test-postgres`.
