...
class ActiveDirectoryIntegration():
	def __init__(self):
		# AD Configuration
		self.AD_SERVER = os.getenv('DC_HOSTNAME', '')
		self.AD_USERNAME = os.getenv('AD_USERNAME', '')
		self.AD_PASSWORD = os.getenv('AD_PASSWORD', '')
		self.DOMAIN_BASE = os.getenv('DOMAIN_BASE', '')
		self.AWAY_GROUPS = os.getenv('AWAY_GROUPS', []).split(";")
		self.HOME_GROUPS = os.getenv('HOME_GROUPS', []).split(";")
	...
	def getGroupName(distinguishedName):
		pattern = re.compile('CN=(.*?),.*')
		match = pattern.match(distinguishedName)
