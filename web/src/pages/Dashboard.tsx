...
function createScheduleColumns({
  ...
  onRequestDelete,
  deletingScheduleId,
}: ScheduleColumnOptions): GridColDef<Schedule>[] {
  return [
    {
      field: "display_name",
      headerName: "User",
  ...
  const handleDeleteSchedule = useCallback(
    async (scheduleId: string) => {
      ...
      try {
        ...
        console.error("Delete schedule failed", deleteError);
      } finally {
  ...
  const handleConfirmDelete = useCallback(
    async (scheduleId: string) => {
      const { confirmed } = await confirm({
        ...
        description:
          "Are you sure you wish to delete this schedule, this operation cannot be undone.",
      ...
      <Stack spacing={3}>
        ...
        <Paper sx={{ height: "70vh", width: "100%" }}>
          <DataGrid
            ...
            slots={{ noRowsOverlay: NoSchedulesOverlay }}
          />
        </Paper>
      </Stack>
      <ScheduleDialog
