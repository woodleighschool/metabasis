...
func TestFailedReconciliationRetainsMeasuredResults(t *testing.T) {
	...
	text := rows.String()
	for _, want := range []string{"✗ Reconciliation  failed", "    ✓ Fetching inventory", "3/3 sources 100%", "    ✗ Planning changes"} {
		if !strings.Contains(text, want) {
			t.Fatalf("missing %q: %s", want, text)
		}
	}
	if strings.Count(text, "Reconciliation") != 1 || strings.Count(text, "Fetching inventory") != 1 || strings.Contains(text, "(0s)") || strings.Contains(text, "·") {
...
func TestSubjectsHaveIndependentOutcomes(t *testing.T) {
	var out bytes.Buffer
	o := &commandOutput{out: &out, interactive: true}
	logger := slog.New(&stageHandler{Handler: slog.NewTextHandler(&out, nil), output: o})
	logger.Info("Loading accepted subjects", "stage", true)
	logger.Info("Loading accepted subjects", "stage_result", true)
	for _, subject := range []string{"first", "second"} {
		logger.Info("Resolving identity", "stage", true, "subject", subject)
		logger.Info("Resolving identity", "stage_result", true, "subject", subject)
		logger.Info("Subject reconciled", "subject_result", true, "subject", subject, "changes", 1)
	}
	o.endProgress(nil)
	if strings.Contains(out.String(), "Reconciliation") {
		t.Fatalf("setup remains among subject results: %s", out.String())
	}
	for _, subject := range []string{"first", "second"} {
		if strings.Count(out.String(), subject) != 1 || !strings.Contains(out.String(), "✓ "+subject+"  applied (1 change)") {
			t.Fatalf("subject output: %s", out.String())
		}
	}
}
