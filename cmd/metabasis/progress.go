...
type activity struct {
	stage, progress, status, final, subjectResult bool
	changes                                       int64
...
func readActivity(record slog.Record, attrs []slog.Attr) activity {
	...
	read := func(attr slog.Attr) bool {
		switch attr.Key {
		case "subject_result":
			a.subjectResult = attr.Value.Kind() == slog.KindBool && attr.Value.Bool()
		case "changes":
			a.changes = activityCount(attr.Value)
		case "stage":
...
func (p *terminalProgress) group(scope string) *progressGroup {
	if scope != "" {
		p.groups = slices.DeleteFunc(p.groups, func(group *progressGroup) bool {
			return group.scope == "" && !slices.ContainsFunc(group.rows[1:], func(line *progressLine) bool {
				return line.ended.IsZero() || line.outcome != "done"
			})
		})
	}
